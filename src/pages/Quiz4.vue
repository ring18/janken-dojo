<template>
  <main>
    <h2>
      ...
      <br>
      <button v-on:click="buttonStop">stop</button>
      <br>
      <span>{{min}}:{{sec}}</span>
      <br>
      ...
      <br>
      <button ref="rock" @click="judge(0)"><!--グーボタン-->
      グー     
      </button>
      <button ref="scissors" @click="judge(1)"><!--チョキボタン-->
      チョキ     
      </button>
      <button ref="paper" @click="judge(2)"><!--パーボタン-->
      パー  
      </button>
      <br>
...
export default {
  ...
  mounted () {  // 初期化時に実行される（らしい）
    ...
    this.buttonInterval = setInterval(() => this.btnPosChange(), 1000);
  },
  components: {
  },
  computed: {
