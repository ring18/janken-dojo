...
export default {
  ...
  methods: {
    ...
    judge(myHand){//storeの手の乱数randと押されたボタンを比較して判定
        if(this.$store.state.mission===0||this.$store.state.mission===2){
        ...
        }else if(this.$store.state.mission===1){
          if((this.$store.state.rand===0 && myHand === 2)||(this.$store.state.rand===1 && myHand === 0)||(this.$store.state.rand===2 && myHand === 1)){
              this.$store.state.missionMessage = "負けてって言ったよね？？";
          }else if (this.$store.state.rand===myHand){
              this.$store.state.missionMessage = "負けてって言ったよね？？";
          }else{
              this.$store.state.missionMessage = "勝たせてくれてありがとう！";
              this.isActive=false;
          }
        }
    }
